...
#Reading the vgdata
#vgdata = read.csv("C:\\Users\\PersonalSpace\\Desktop\\MSIS 5223 PDS2\\project\\vgdata.txt", stringsAsFactors = FALSE)
...
# Regression Model 1
new_reg1 = lm(reg_data$sales ~ reg_data$metascore + reg_data$console_PS4 + 
                reg_data$console_XBox1 + reg_data$console_Switch)
summary(new_reg1)

# Regression Model 2
new_reg2 = lm(reg_data$sales ~ reg_data$metascore + reg_data$genre_Platform + 
                reg_data$genre_Racing + reg_data$genre_RPG + reg_data$genre_Shooter + 
                reg_data$genre_Sports + reg_data$genre_Action_Adventure + reg_data$genre_Misc)
summary(new_reg2)

# Regression Model 3
new_reg3 = lm(reg_data$sales ~ reg_data$metascore + reg_data$player_vector + 
                reg_data$esrb_vector +  reg_data$user_score_x10
              + reg_data$igdb_member_rating + reg_data$igdb_critic_rating + reg_data$ign_x10
              +  reg_data$mgAverage)
summary(new_reg3)

# Regression Model 4
new_reg4 = lm(reg_data$sales ~ reg_data$metascore  + 
                reg_data$genre_Shooter +  reg_data$genre_Sports + reg_data$user_score_x10)
summary(new_reg4)

# Regression Model 5
new_reg5 = lm(reg_data$sales ~ reg_data$metascore + reg_data$genre_Shooter + reg_data$user_score_x10)
summary(new_reg5)
